...
// Owner-supplied text ends up inside HTML that customers load, so it is escaped.
// Without this, a welcome message containing a < would break the page, and a
// malicious one could run script in every customer's browser.
function esc(s) {
  return String(s || "")
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
...
// Colours are dropped straight into CSS, so only allow real hex colours.
function safeColor(c) {
  return /^#[0-9a-fA-F]{3,8}$/.test(String(c || "")) ? c : "#0b2447";
}
...
// The one place the portal's name is decided, used by BOTH pages so the login
// screen and the "you're connected" screen can never disagree again.
function brandLine(t) {
  return t.portal_welcome ? esc(t.portal_welcome) : `Welcome to ${esc(t.business_name || "our")} WiFi`;
}

function pageTitle(t) {
  return esc(t.portal_welcome || t.business_name || "WiFi");
}

function buildLoginPage(t, origin, tenantId) {
  const color = safeColor(t.brand_color);
  const logo = t.portal_logo_url
    ? `<img src="${esc(t.portal_logo_url)}" alt="" style="max-height:64px;margin-bottom:12px">`
    : "";
  const support = t.support_phone
    ? `<p class="support">Need help or want to buy a code? Call/WhatsApp: <b>${esc(t.support_phone)}</b></p>` : "";
  return `<html>
<head><title>${pageTitle(t)}</title>
<meta name="viewport" content="width=device-width, initial-scale=1">
<style>
body{margin:0;font-family:-apple-system,Segoe UI,Roboto,Arial,sans-serif;background:${color};min-height:100vh;display:flex;align-items:center;justify-content:center}
...
button{width:100%;margin-top:12px;padding:13px;border:0;border-radius:10px;background:${color};color:#fff;font-size:16px;font-weight:700;cursor:pointer}
.err{color:#c0392b;font-size:13px;min-height:16px;margin-top:8px}
.buy{display:block;margin-top:14px;font-size:14px;color:${color};font-weight:600;text-decoration:none}
...
.fill{background:${color};height:100%;width:0%}
...
<p>${brandLine(t)}. Enjoy your browsing.</p>
